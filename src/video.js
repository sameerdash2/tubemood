...
class Video {
    ...
    shouldReFetch = (row) => {
        logger.log('info', "Re-fetching video %s.", this._id);
        return true;
    }
                    ...
                    if (inDeletion) {
                        ...
                        if (!actuallyInProgress) {
                            ...
                            if (row.nextPageToken) {
                                ...
                                this._indexedComments = row.commentCount ?? 0;
    ...
    fetchAllComments(pageToken, appending, consecutiveErrors = 0) {
        ...
        Promise.race([timeoutHolder, this._app.ytapi.executeCommentChunk(this._id, pageToken)]).then((response) => {
            ...
            if (
                ...
                && this._indexedComments < this._commentCount
    ...
    requestLoadedComments(sort, commentIndex, pageSize, broadcast, minDate, maxDate, isFirstBatch = false) {
        ...
        try {
            ...
            // Compute statistics and send them with the first batch.
            if (isFirstBatch) {
